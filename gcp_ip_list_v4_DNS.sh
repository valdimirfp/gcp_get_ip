...
do
 for zn in $gcp_zones
 do
   gcloud dns record-sets list --zone $zn --filter TYPE=A --project $pr -q 2> /dev/null >> gcp_ip_A_dns_"$(date +"%d-%m-%Y")".txt
 done
done
...
awk '!x[$0]++' gcp_ip_A_dns_* | sed '/^10/d' >> gcp_ip_A_dns_"$(date +"%d-%m-%Y")"_DNS.txt
